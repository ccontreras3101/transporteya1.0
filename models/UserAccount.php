...
class UserAccount extends AccountModel implements \yii\web\IdentityInterface
{
    ...
    public static function findByUsername($username, $password)
    {
        ...
        $password = self::encodeLoginPassword($password);
     ...
     * @return string
     */
    public static function encodeLoginPassword($password)
    {
        if (($sessionData = self::getSessionData()) && !empty($sessionData['sessionhandle'])) {
            $sessionHandle = base64_encode(hash('sha512', $sessionData['sessionhandle'], true));
            $password = hash('sha512', $password);

            return base64_encode(hash('sha512', $password . $sessionHandle, true));
        }
